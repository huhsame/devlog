...
**디자인 시스템을 코드로 먼저 정의하세요.** "이런 느낌으로 해줘"라고 말로 하면 매번 다른 결과가 나옵니다. `.highlight`, `.hand-drawn-border` 같은 유틸리티 클래스를 미리 만들어놓고 "이 클래스를 써"라고 하면, 컴포넌트가 10개든 20개든 일관성이 유지됩니다.
...
**컨벤션을 문서화하세요.** `MEMORY.md`에 "Supabase 클라이언트는 이렇게 쓴다", "Admin API는 이 패턴을 따른다"를 적어두면, 세션이 바뀌어도 일관된 코드가 나옵니다. AI한테 매번 같은 설명을 반복하지 않아도 됩니다.
