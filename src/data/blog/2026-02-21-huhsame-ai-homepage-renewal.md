...
- 허세임 플랫폼(huhsame.com)을 "강의 판매 사이트"에서 "공부하는 공간"으로 전환. 블로그/튜토리얼/쇼케이스 구조 리뉴얼
- "손으로 그린 기술" 디자인 시스템을 CSS 변수로 먼저 정의하고, 전체 컴포넌트에 일관 적용. 라이트 모드(강의 노트)와 다크 모드(칠판과 분필)로 분위기 전환
- 15,000자 브리핑 + Plan 모드로 5개 Phase 설계 후 순차 구현하는 바이브코딩 워크플로우 시도
- 브랜드 스킬/에이전트 파이프라인 구성으로 디자인 가이드 → 코드 생성 → 검증까지 재사용 가능한 구조 확보
- MEMORY.md로 세션 간 컨벤션 유지, shadcn 리스타일 패턴으로 기존 컴포넌트 위에 브랜드 입히기
